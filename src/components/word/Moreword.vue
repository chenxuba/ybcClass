<template>
  <div>
    <van-row class="box-warp" v-if="word != ''">
      <van-row class="msg_warp" v-for="(item,index) in word" :key="index" @click="goDetail(item.id)">
        <van-col span="9">
          <div class="img_warp">
            <img v-lazy="item.pic_cover" v-if="item.pic_cover" alt />
        ...
        <van-col span="15" class="right">
          ...
          <div class="top2">
            <span id="about_text" v-if="item.abstract != ' '">{{item.abstract}}</span>
            <span id="about_text" v-else>暂时没有简介，点击我进入看全文～</span>
          </div>
          <div class="bottom">
            <van-row>
              <van-col span="8">
    ...
    <img src="../../../static/img/sholist.svg" v-for="item in 8" alt :key="item" v-else />
  </div>
</template>
