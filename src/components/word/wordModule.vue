<template>
  <div>
    <van-row class="box-warp" v-if="word.info">
      <van-col span="24" class="hot_warp">
        <van-row type="flex" justify="space-between" class="warp1">
          <van-col span="10" class="title">{{word.title}}</van-col>
          <van-col span="6" class="more" @click="goWordModuleDeatil(name)">更多</van-col>
        </van-row>
      </van-col>
      <van-row class="msg_warp" v-for="(item,index) in word.info" :key="index" @click="goDetail(item.id)">
        <van-col span="9">
          <div class="img_warp">
    ...
    <van-divider :style="{ color: '#1989fa', borderColor: '#1989fa', padding: '0 16px' }"><span @click="goWordModuleDeatil(name)">点击查看更多</span> </van-divider>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    goWordModuleDeatil(name) {
      this.$router.push({
        path: "/wordModuleDeatil",
        query: {
          type: this.word.type,
          label_id: this.word.label_id
        }
      });
    },
    goDetail(id){
