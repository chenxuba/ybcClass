<template>
  <div class="applyTeacher" v-height>
    ...
    <div class="binding background" v-if="isChongxinShenqing">
      <div class="warp" @click="$router.push('/openTeacher')">
        <span>重新申请</span>
        <span>重新申请导师</span>
      </div>
    </div>
    ...
    <div class="binding background">
      <div class="warp" @click="$router.push('/bdingteacher')">
        <span>绑定导师</span>
        <span>已有账号立即绑定</span>
      </div>
    </div>
    ...
    async checkResult() {
      ...
      if (result.code == 1) {
        ...
        if (result.data.status == 4) {
          ...
          this.$router.push("/teacherUserinfo");
        } else if (result.data.status == 5) {
          this.text = "您的导师账号已被冻结";          
