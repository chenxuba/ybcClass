  ...
  <div class="cellView">
      <div class="box">
        ...
        <div class="clock">
          ...
          <!-- 提问按钮 -->
          <div class="btn btn2" v-if="queryObj.answer.reply == 0">
            <van-button
              plain
              tag="button"
              type="primary"
              size="small"
              :to="'/ask/'+queryObj.user_id"
            >导师暂未回复，点击继续提问</van-button>
          </div>
          <div class="btn btn-1" v-if="queryObj.answer.reply == -1">
            <span>72小时未回复,费用已原路返回</span>
          </div>
          <div class="btn btn3" v-if="queryObj.answer.reply == 1">
            <span>{{queryObj.watch_num}}人围观过</span>
          </div>
        </div>
        <!-- 自己的问题，导师回答后的答案 -->
        <div class="pupop" v-if="queryObj.is_apply == 2">
